...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    async def get_plant(call: ServiceCall) -> ServiceResponse:
        ...
        if not entries:
            raise OpenPlantbookException("No config entry found")
        entry = entries[0]
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(breeder_url) as response:
                    if response.status != 200:
                        raise OpenPlantbookException(f"HTTP error {response.status} for URL {breeder_url}")
            ...
            if not table:
                raise OpenPlantbookException(f"No strain table found for breeder {breeder}")
            ...
            if not strain_data:
                raise OpenPlantbookException(f"Strain {species} not found for breeder {breeder}")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(strain_data['link']) as response:
                    if response.status != 200:
                        raise OpenPlantbookException(f"HTTP error {response.status} for URL {strain_data['link']}")
            ...
            raise OpenPlantbookException(f"Error fetching data: {str(ex)}")
